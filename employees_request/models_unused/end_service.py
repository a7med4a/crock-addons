...
class end_service(models.Model):
    ...
    gross = fields.Float(string="Gross", tracking=True, compute="cal_gross_amt")
    alwns_amount = fields.Float(
        string="Allowances", tracking=True, compute="cal_alwns_amt"
    )
    loan_status = fields.Selection(
        string="Loan status", related="employee_id.loan_status"
    )
    date_last_bouns = fields.Date(
        string="Date Last End of Service Gratuity",
        required=False,
        related="employee_id.date_last_bouns",
    )

    bonus_value = fields.Integer(
        string="Bonus Value", readonly=True, states={"draft": [("readonly", False)]}
    )
    start_service_period = fields.Date(
        string="Period Start Service",
        readonly=True,
        states={"draft": [("readonly", False)]},
    )
    end_service_period = fields.Date(
        string="Period End of Service",
        readonly=True,
        states={"draft": [("readonly", False)]},
    )
    ...
    debit_account_id = fields.Many2one(
        ...
        readonly=True,
        states={"draft": [("readonly", False)]},
        default=lambda self: self.env.company.req_endserv_acc_debit_id.id,
    )
    credit_account_id = fields.Many2one(
        ...
        readonly=True,
        states={"draft": [("readonly", False)]},
        default=lambda self: self.env.company.req_endserv_acc_credit_id.id,
    )
    journal_id = fields.Many2one(
        ...
        readonly=True,
        states={"draft": [("readonly", False)]},
        default=lambda self: self.env.company.req_endserv_journal_id.id,
    )
    ...
    def cal_alwns_amt(self):
        for rec in self:
            alw_catg = (
                ...
                .sudo()
                .search([("code", "=", "ALW")], limit=1, order="id DESC")
            )
            payslip = (
                self.env["hr.payslip"]
                .sudo()
            ...
            alw_lines = payslip.line_ids.filtered(
                lambda record: record.category_id.id == alw_catg.id
            )
            if alw_lines:
    ...
    def create(self, values):
        values["name"] = self.env["ir.sequence"].next_by_code("service.order") or _(
            "New"
        )
